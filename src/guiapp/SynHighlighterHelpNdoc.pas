...
type
  TtkTokenKind = (tkComment, tkIdentifier,
    tkKey, tkSubKey,
    tkNull, tkNumber,
...
const
   cDefaultIdentChars = '_\@0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ' +
                         'abcdefghijklmnopqrstuvwxyz';

type
  TSynHelpNdocSyn = class(TSynCustomHighlighter)
  private
    ...
    fSubKeyAttri: TSynHighlighterAttributes;
    ...
    procedure SetSubKeyWords(const Value: TUnicodeStrings);
  ...
  public
    ...
    function IsSubKeyword(const AKeyword: UnicodeString): Boolean;
  ...
  published
    ...
    property SubKeyAttri: TSynHighlighterAttributes read fSubKeyAttri write fSubKeyAttri;
    property SubKeyWords: TUnicodeStrings read fSubKeyWords write SetSubKeyWords;
...
const
  SYN_ATTR_SUBKEYWORD        =  10;
  SYNS_AttrReservedWordSub      =  'ReservedWordSub';
  SYNS_FriendlyAttrReservedWordSub      =  'Reserved Word Sub';

function TSynHelpNdocSyn.IsIdentChar(AChar: WideChar): Boolean;
var
  i: Integer;
begin
  Result := False;
...
function TSynHelpNdocSyn.IsKeyword(const AKeyword: UnicodeString): Boolean;
var
...
function TSynHelpNdocSyn.IsSubKeyword(const AKeyword: UnicodeString): Boolean;
var
  First, Last, I, Compare: Integer;
  Token: UnicodeString;
begin
  First := 0;
  Last := fSubKeywords.Count - 1;
  Result := False;
  Token := SynWideUpperCase(AKeyword);
  while First <= Last do
  begin
    I := (First + Last) shr 1;
    Compare := WideCompareText(fSubKeywords[i], Token);
    if Compare = 0 then
    begin
      Result := True;
      break;
...
function TSynHelpNdocSyn.IsWordBreakChar(AChar: WideChar): Boolean;
begin
  Result := inherited IsWordBreakChar(AChar) and not IsIdentChar(AChar);
end;

constructor TSynHelpNdocSyn.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);

  fKeyWords := TUnicodeStringList.Create;
  fSubKeyWords := TUnicodeStringList.Create;

  TUnicodeStringList(fKeyWords).Sorted := True;
  TUnicodeStringList(fKeyWords).Duplicates := dupIgnore;

  TUnicodeStringList(fSubKeyWords).Sorted := True;
  TUnicodeStringList(fSubKeyWords).Duplicates := dupIgnore;
  ...
  fSubKeyAttri := TSynHighlighterAttributes.Create(SYNS_AttrReservedWordSub, SYNS_FriendlyAttrReservedWordSub);
  fSubKeyAttri.Style := [fsBold];
  AddAttribute(fSubKeyAttri);
...
destructor TSynHelpNdocSyn.Destroy;
begin
  fKeyWords.Free;
  fSubKeyWords.Free;
  inherited Destroy;
...
procedure TSynHelpNdocSyn.AnsiProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynHelpNdocSyn.PasStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynHelpNdocSyn.CStyleProc;
begin
  case fLine[Run] of
     #0: NullProc;
    #10: LFProc;
...
procedure TSynHelpNdocSyn.AsciiCharProc;
begin
...
procedure TSynHelpNdocSyn.BraceOpenProc;
begin
...
procedure TSynHelpNdocSyn.PointCommaProc;
begin
...
procedure TSynHelpNdocSyn.CRProc;
begin
  fTokenID := tkSpace;
  Inc(Run);
...
procedure TSynHelpNdocSyn.IdentProc;
begin
...
procedure TSynHelpNdocSyn.SubIdentProc;
begin
  while IsIdentChar(fLine[Run]) do inc(Run);
  if IsKeyWord(GetToken) then
    fTokenId := tkSubKey
...
procedure TSynHelpNdocSyn.IntegerProc;
...
procedure TSynHelpNdocSyn.LFProc;
begin
  fTokenID := tkSpace;
  inc(Run);
end;

procedure TSynHelpNdocSyn.NullProc;
begin
  fTokenID := tkNull;
  inc(Run);
end;

procedure TSynHelpNdocSyn.NumberProc;
...
procedure TSynHelpNdocSyn.RoundOpenProc;
begin
  inc(Run);
...
procedure TSynHelpNdocSyn.SlashProc;
begin
  Inc(Run);
  case FLine[Run] of
    '/':
...
procedure TSynHelpNdocSyn.SpaceProc;
begin
  inc(Run);
  fTokenID := tkSpace;
...
procedure TSynHelpNdocSyn.StringProc;
var
   delim : WideChar;
begin
  fTokenID := tkString;
...
procedure TSynHelpNdocSyn.UnknownProc;
begin
  inc(Run);
  fTokenID := tkUnknown;
end;

procedure TSynHelpNdocSyn.Next;
begin
  fTokenPos := Run;
  case fRange of
    rsAnsi: AnsiProc;
...
function TSynHelpNdocSyn.GetDefaultAttribute(Index: integer): TSynHighlighterAttributes;
begin
  case Index of
    ...
    SYN_ATTR_SUBKEYWORD: Result := fSubKeyAttri;
...
function TSynHelpNdocSyn.GetEol: Boolean;
begin
  Result := Run = fLineLen + 1;
end;

function TSynHelpNdocSyn.GetRange: Pointer;
begin
  Result := Pointer(fRange);
end;

function TSynHelpNdocSyn.GetTokenID: TtkTokenKind;
begin
  Result := fTokenId;
end;
...
// GetCharBeforeToken
//
function TSynHelpNdocSyn.GetCharBeforeToken(offset : Integer = -1) : WideChar;
begin
...
// GetCharAfterToken
//
function TSynHelpNdocSyn.GetCharAfterToken(offset : Integer = 1) : WideChar;
begin
   Result:=FLine[fTokenPos+offset];
end;

function TSynHelpNdocSyn.GetTokenAttribute: TSynHighlighterAttributes;
begin
  case fTokenID of
...
function TSynHelpNdocSyn.GetTokenKind: integer;
begin
  Result := Ord(fTokenId);
end;

procedure TSynHelpNdocSyn.ResetRange;
begin
  fRange := rsUnknown;
end;

procedure TSynHelpNdocSyn.SetRange(Value: Pointer);
begin
  fRange := TRangeState(Value);
end;

procedure TSynHelpNdocSyn.SetKeyWords(const Value: TUnicodeStrings);
var
  i: Integer;
begin
  if Value <> nil then
...
procedure TSynHelpNdocSyn.SetSubKeyWords(const Value: TUnicodeStrings);
var
  i: Integer;
begin
  if Value <> nil then
  ...
  TUnicodeStringList(fSubKeyWords).Sorted:=False;
  fSubKeyWords.Assign(Value);
  TUnicodeStringList(fSubKeyWords).Sorted:=True;

  DefHighLightChange(nil);
end;

procedure TSynHelpNdocSyn.SetComments(Value: TCommentStyles);
begin
...
class function TSynHelpNdocSyn.GetLanguageName: string;
begin
  Result := SYNS_LangGeneral;
end;

{$IFNDEF SYN_CLX}
function TSynHelpNdocSyn.LoadFromRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
...
function TSynHelpNdocSyn.SaveToRegistry(RootKey: HKEY; Key: string): boolean;
var
  r: TBetterRegistry;
begin
  r:= TBetterRegistry.Create;
...
function TSynHelpNdocSyn.GetStringDelim: TStringDelim;
begin
  Result:=fStringDelim;
end;

procedure TSynHelpNdocSyn.SetStringDelim(const Value: TStringDelim);
begin
   fStringDelim:=Value;
end;

function TSynHelpNdocSyn.GetIdentifierChars: UnicodeString;
begin
  Result := fIdentChars;
end;

procedure TSynHelpNdocSyn.SetIdentifierChars(const Value: UnicodeString);
begin
  fIdentChars := Value;
end;

function TSynHelpNdocSyn.StoreIdentChars : Boolean;
begin
   Result := (fIdentChars<>cDefaultIdentChars);
end;

procedure TSynHelpNdocSyn.SetDetectPreprocessor(Value: boolean);
begin
...
class function TSynHelpNdocSyn.GetFriendlyLanguageName: UnicodeString;
begin
...
// IsStringDelim
//
function TSynHelpNdocSyn.IsStringDelim(aChar : WideChar) : Boolean;
begin
...
procedure Register;
begin
  RegisterComponents('KALLUP', [TSynHelpNdocSyn]);
end;
